...
class _SheetDataScreenState extends ConsumerState<SheetDataScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Uploaded Cards"),
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh),
            onPressed: () => ref.refresh(sheetDataProvider),
          ),
        ],
      ),
      body: Column(
        children: [
          ...
          Expanded(
            child: RefreshIndicator(
              onRefresh: () async => ref.refresh(sheetDataProvider.future),
  ...
  Widget _buildList(BuildContext context, List<dynamic> data) {
    if (data.isEmpty) {
      return _buildScrollablePlaceholder(context, Icons.folder_open, "No cards uploaded yet");
    }
